#!/usr/bin/env python3
"""Fail-closed build-time integration patch for DXWeaver 0.5.2.
...
without changing the legacy compiler dialect. The v0.5.2 patch keeps the
v0.5.1 cockpit/layout correction and fixes HUNT target activation by routing
ranked CQ stations through MSHV's native decode-selection/double-click path.
"""
...
def apply(upstream: Path) -> None:
    ...
    native_autoseq = """    // DXWeaver 0.5.2: decoder -> intelligence -> native MSHV selection/AutoSeq, entirely in-process.
...
"""
    ...
    # Main_Ms is a QWidget and already owns one master QVBoxLayout (V_l). Put
    # DXWeaver inside that layout; never float it over the waterfall with
    # absolute coordinates or z-order manipulation.
    replace_once(
        main_cpp,
        "    V_l->setSpacing(0);\n",
        "    V_l->setSpacing(2);\n",
